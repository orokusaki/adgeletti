...
class Size(models.Model):
    ...
    width = models.PositiveIntegerField(
        _(u'width'), help_text=_(u'The width of an ad, in pixels.'))
    height = models.PositiveIntegerField(
        _(u'height'), help_text=_(u'The height of an ad, in pixels.'))
...
class AdUnit(models.Model):
    ...
    ad_unit = models.CharField(
        _(u'Ad unit ID'), max_length=255, unique=True,
        help_text=_(u'The ad unit ID, without the network ID'))
...
class AdPosition(models.Model):
    ...
    slot = models.CharField(
        _(u'slot'), max_length=255,
        choices=[(slot, slot) for slot in settings.ADGELETTI_SLOTS])
    breakpoint = models.CharField(
        _(u'breakpoint'), max_length=255,
        choices=[(bp, bp) for bp in settings.ADGELETTI_BREAKPOINTS])
    sizes = models.ManyToManyField(Size, verbose_name=_(u'allowed sizes'))

    class Meta:
        unique_together = ('ad_unit', 'slot', 'breakpoint')
